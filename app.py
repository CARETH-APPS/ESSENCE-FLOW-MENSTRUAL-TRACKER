...
<style>
    ...
    /* ─── BASE MOBILE OVERRIDES (applied only when .mobile-mode is present) ─── */
    ...
    .mobile-mode .main .block-container {
        padding: 0.2rem !important;
    }

    /* Mobile-specific: shrink everything and keep columns in a row */
    .mobile-mode .row-widget.stColumns {
        flex-wrap: nowrap !important;
    }
    .mobile-mode .row-widget.stColumns > div {
        flex: 1 1 0 !important;
        min-width: 0 !important;
        max-width: 100% !important;
        padding: 0.1rem 0.15rem !important;
    }
    .mobile-mode .row-widget.stColumns > div .stButton button {
    ...
    .mobile-mode h1 { font-size: 1.0rem !important; }
    .mobile-mode h2 { font-size: 0.85rem !important; }
    .mobile-mode h3 { font-size: 0.75rem !important; }
    .mobile-mode p, .mobile-mode li, .mobile-mode .stMarkdown, .mobile-mode .stCaption, .mobile-mode .stMetric label {
        font-size: 0.65rem !important;
        line-height: 1.2 !important;
    }
    .mobile-mode .stMetric .stMetricValue {
        font-size: 1.1rem !important;
    }
    .mobile-mode .stButton button {
    ...
    .mobile-mode .card, .mobile-mode .quick-card, .mobile-mode .alert-banner {
        padding: 0.4rem !important;
        margin: 0.2rem 0 !important;
        border-radius: 12px !important;
        width: 100% !important;
    }
    .mobile-mode .stExpander { width: 100% !important; }
    .mobile-mode .stDataFrame { overflow-x: auto !important; }
    .mobile-mode .stMetric { text-align: center !important; }
    .mobile-mode section[data-testid="stSidebar"] {
        width: 80vw !important;
        max-width: 260px !important;
    }
    .mobile-mode section[data-testid="stSidebar"][aria-expanded="false"] + section {
        margin-left: 0 !important;
        width: 100% !important;
    }
    .mobile-mode .book-reader {
    ...
    .mobile-mode .stTextInput input, .mobile-mode .stNumberInput input, .mobile-mode .stSelectbox select, .mobile-mode .stTextArea textarea {
        font-size: 0.6rem !important;
        padding: 0.2rem !important;
        min-height: 28px !important;
    }
    .mobile-mode .stDateInput input {
        font-size: 0.6rem !important;
        padding: 0.2rem !important;
    }
    .mobile-mode .stSlider > div {
        padding: 0.2rem 0 !important;
    }
    .mobile-mode .stSlider .stSliderValue {
        font-size: 0.6rem !important;
    }
    .mobile-mode .stCheckbox label, .mobile-mode .stRadio label {
        font-size: 0.6rem !important;
    }
    /* Extra small (≤400px) – even more shrink */
    @media (max-width: 400px) {
        ...
        .mobile-mode h1 { font-size: 0.85rem !important; }
        .mobile-mode h2 { font-size: 0.7rem !important; }
        .mobile-mode .card, .mobile-mode .quick-card { padding: 0.2rem !important; }
    }
</style>
""", unsafe_allow_html=True)
...
def detect_and_set_device():
    """
    Injects JavaScript to detect mobile device via user-agent and
    sets a query parameter 'device' to 'mobile' or 'desktop'.
    """
    if "device" not in st.query_params:
        # First load – inject script that will redirect with the device param
        st.markdown("""
        <script>
        (function() {
            const ua = navigator.userAgent || navigator.vendor || window.opera;
            // Detect mobile using common patterns
            const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(ua);
            const device = isMobile ? 'mobile' : 'desktop';
            // Check if the param is already set
            const url = new URL(window.location.href);
            if (url.searchParams.get('device') !== device) {
        ...
        """, unsafe_allow_html=True)
        st.stop()  # Halt execution until the page reloads with the param

    # After reload, read the param and store in session_state
    device = st.query_params.get("device", "desktop")
    st.session_state.device = device

# ─── CONDITIONAL MOBILE CSS INJECTION ──────────────────────────────
def apply_mobile_css_if_needed():
    """Inject the mobile-mode class if device is mobile."""
    if st.session_state.get("device") == "mobile":
        # The class is already applied via the static CSS above using .mobile-mode,
        # but we need to add the class to the body. We'll do that with a tiny script.
        st.markdown("""
        <script>
...
def main():
    ...
    # ─── APPLY MOBILE CSS IF DETECTED ────────────────────────────
    apply_mobile_css_if_needed()
